import pytest
from pgcli.packages.parseutils import (extract_tables, TableReference,
    find_prev_keyword, is_open_quote)
...
def test_simple_select_single_table():
    ...
    assert tables == (TableReference(None, 'abc', None, False),)
...
def test_simple_select_single_table_schema_qualified(sql):
    ...
    assert tables == (TableReference('abc', 'def', None, False),)
...
def test_simple_select_multiple_tables():
    ...
    assert set(tables) == set([TableReference(None, 'abc', None, False),
...
def test_simple_select_single_table_deouble_quoted_aliased():
    ...
    assert tables == (TableReference(None, 'Abc', 'a', False),)

def test_simple_select_multiple_tables_deouble_quoted_aliased():
    tables = extract_tables('select * from "Abc" a, "Def" d')
    assert set(tables) == set([TableReference(None, 'Abc', 'a', False),
                               TableReference(None, 'Def', 'd', False)])

def test_simple_select_multiple_tables_schema_qualified():
    tables = extract_tables('select * from abc.def, ghi.jkl')
    assert set(tables) == set([TableReference('abc', 'def', None, False),
                               TableReference('ghi', 'jkl', None, False)])
...
def test_simple_select_with_cols_multiple_qualified_tables():
    ...
    assert set(tables) == set([TableReference('abc', 'def', None, False),
                               TableReference('def', 'ghi', None, False)])
...
def test_select_with_hanging_period_multiple_tables():
    ...
    assert set(tables) == set([TableReference(None, 'tabl1', 't1', False),
                               TableReference(None, 'tabl2', 't2', False)])
...
def test_simple_insert_single_table():
    ...
    # assert tables == (TableReference(None, 'abc', 'abc', False),)
...
def test_join_as_table():
    ...
    assert tables == (TableReference(None, 'my_table', 'm', False),)
...
def test_multiple_joins():
    ...
    tables = extract_tables(sql)
    assert tables == (
        TableReference(None, 't1', None, False),
        TableReference(None, 't2', None, False),
        TableReference(None, 't3', None, False))
