...
from pgcli.packages.parseutils import TableReference
import pytest
...
def test_select_suggests_cols_with_visible_table_scope():
    ...
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'tabl', None, False),)),
...
def test_select_suggests_cols_with_qualified_table_scope():
    ...
    assert set(suggestions) == set([
        Column(tables=(TableReference('sch', 'tabl', None, False),)),
        Function(schema=None),
        Keyword()
    ])
...
def test_lparen_suggests_cols():
    ...
    assert set(suggestion) == set([
        Column(tables=(TableReference(None, 'tbl', None, False),))])
...
def test_insert_into_lparen_suggests_cols():
    ...
    assert suggestions ==(Column(tables=(TableReference(None, 'abc', None, False),)),)
...
def test_dot_suggests_cols_of_an_alias(sql):
    ...
    assert set(suggestions) == set([
        Table(schema='t1'),
        View(schema='t1'),
        Column(tables=(TableReference(None, 'tabl1', 't1', False),)),
        Function(schema='t1'),
    ])
...
def test_dot_col_comma_suggests_cols_or_schema_qualified_table():
    suggestions = suggest_type('SELECT t1.a, t2. FROM tabl1 t1, tabl2 t2',
            'SELECT t1.a, t2.')
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'tabl2', 't2', False),)),
        Table(schema='t2'),
        View(schema='t2'),
        Function(schema='t2'),
    ])
...
def test_outer_table_reference_in_exists_subquery_suggests_columns():
    ...
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'foo', 'f', False),)),
        Table(schema='f'),
        View(schema='f'),
        Function(schema='f'),
    ])
...
def test_sub_select_col_name_completion():
    suggestions = suggest_type('SELECT * FROM (SELECT  FROM abc',
            'SELECT * FROM (SELECT ')
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'abc', None, False),)),
        Function(schema=None),
        Keyword(),
    ])
...
def test_sub_select_dot_col_name_completion():
    ...
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'tabl', 't', False),)),
        Table(schema='t'),
        View(schema='t'),
        Function(schema='t'),
    ])
...
def test_join_alias_dot_suggests_cols1(sql):
    suggestions = suggest_type(sql, sql)
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'abc', 'a', False),)),
        Table(schema='a'),
        View(schema='a'),
        Function(schema='a'),
        JoinCondition(tables=(TableReference(None, 'abc', 'a', False),
            TableReference(None, 'def', 'd', False)), lefttable=(
            TableReference(None, 'abc', 'a', False),))
    ])
...
def test_join_alias_dot_suggests_cols2(sql):
    ...
    assert set(suggestion) == set([
        Column(tables=(TableReference(None, 'def', 'd', False),)),
        Table(schema='d'),
        View(schema='d'),
        Function(schema='d'),
    ])
...
def test_on_suggests_aliases_and_join_conditions(sql):
    suggestions = suggest_type(sql, sql)
    assert suggestions == (JoinCondition(tables=(
        TableReference(None, 'abc', 'a', False),
        TableReference(None, 'bcd', 'b', False)),
        lefttable=None),
        Alias(aliases=('a', 'b',)),)
...
def test_on_suggests_tables_and_join_conditions(sql):
    suggestions = suggest_type(sql, sql)
    assert suggestions == (JoinCondition(tables=(
        TableReference(None, 'abc', None, False),
        TableReference(None, 'bcd', None, False)),
        lefttable=None),
...
def test_on_suggests_tables_and_join_conditions_right_side(sql):
    suggestions = suggest_type(sql, sql)
    assert suggestions == (JoinCondition((
        TableReference(None, 'abc', None, False),
        TableReference(None, 'bcd', None, False)),
        None
      ),
...
def test_join_using_suggests_common_columns(col_list):
    ...
    assert set(suggest_type(text, text)) == set([
        Column(tables=(
            TableReference(None, 'abc', None, False),
            TableReference(None, 'def', None, False)),
            drop_unique=True),
    ])
...
def test_suggest_columns_after_multiple_joins():
    sql = '''select * from t1
            ...
            inner join t3 ON
              t2.id = t3.'''
    suggestions = suggest_type(sql, sql)
    assert Column(tables=(TableReference(None, 't3', None, False),)) in set(suggestions)
...
def test_2_statements_2nd_current():
    ...
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'b', None, False),)),
        Function(schema=None),
        Keyword()
    ])
...
def test_2_statements_1st_current():
    ...
    suggestions = suggest_type('select  from a; select * from b',
                               'select ')
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'a', None, False),)),
        Function(schema=None),
        Keyword()
    ])
...
def test_suggest_where_keyword(text):
    ...
    suggestions = suggest_type(text, text)
    assert set(suggestions) == set([
        Column(tables=(TableReference(None, 'foo', None, False),)),
        Function(schema=None),
        Keyword(),
    ])
...
def test_select_suggests_fields_from_function():
    ...
    assert set(suggestions) == set([
            Column(tables=(TableReference(None, 'func', None, True),)),
            Function(schema=None),
            Keyword()
            ])
